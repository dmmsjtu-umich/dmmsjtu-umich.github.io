...
When $w=0$, Problem~\ref{tooc:problem:general} becomes the regular OC problem with a fixed planning horizon.
Without stage and terminal costs $\phi(x_T)=0,\ell(x_k,u_k)=0, k=0,1,\cdots,T-1$ and $w > 0$, Problem~\ref{tooc:problem:general} becomes the regular horizon-optimal OC problem.

When the dynamics and cost terms are nonlinear and twice differentiable, Problem \ref{tooc:problem:general} can be iteratively approximated using Taylor expansion and solved using DDP/iLQR.
For the rest of the paper, all our discussion on Problem \ref{tooc:problem:general} relies on the following Assumption.
...
\subsection{HO-LQR Problem}
We first consider a simplified variant of Problem~\ref{tooc:problem:general}.
...
Let $Q_k \succeq 0$ and $R_k \succ 0$ denote the positive semi-definite state cost matrix and positive definite control cost matrix, respectively.
Consider the system with linear dynamics $x_{k+1}=A_k x_k + B_k u_k$.
The stage cost $\ell(x_k,u_k) = x_k^\top Q_k x_k + u_k^\top R_k u_k$ and terminal cost $x_T^\top Q_T x_T$ are both quadratic.
Problem~\ref{tooc:problem:general} becomes Horizon-Optimal Linear Quadratic Regulator (HO-LQR) problem.

\begin{problem}[HO-LQR Problem]\label{tooc:problem:tolqr}
