...
\subsection{Fixed Horizon LQR and Riccati Recursion}
...
\begin{figure}[tb]
    ...
    \vspace{-5mm}
    \caption{Illustration of the ``shift horizon'' one pass approach in \cite{stachowicz2021ohmp} for time-invariant HO-LQR. The map $g$ can be computed based on $(A,B,Q,R)$ matrices, and is identical for all steps $k$. One backward pass yields all $\{P_k\}$, which allows computing $J_k$ by shifting the terminal index. For example, for horizon $k=N$ (blue), the corresponding cost $J_N$ can be computed by using $P_0$. For horizon $k=N-1$ (green), the corresponding cost $J_{N-1}$ can be computed by using $P_1$. The optimal horizon can be computed by finding the horizon $k^*$ such that $J_{k^*}$ reaches the minimum among $J_k$ for all possible $k$.}
    \label{fig:ti_reuse}
    \vspace{-5mm}
\end{figure}

\subsection{Horizon-Optimal Time-Invariant LQR}
% As opposed to fixing the planning horizon $T$ in LQR, when $T$ is also to be optimized, the existing approach in the literature~\cite{stachowicz2021ohmp} only considers the time-invariant case, and it is challenging to generalize this approach to the more general time-varying case.
As opposed to fixing the planning horizon $T$ in LQR, $T$ can also be optimized for systems with stationary dynamics and costs~\cite{stachowicz2021ohmp}.
Specifically, let $g$ denote a mapping corresponding to the computation process $P_k = g(P_{k+1})$ which obtains $P_k$ from $P_{k+1}$.
When the system dynamics satisfy $A_k=A,\forall k$, $B_k=B,\forall k$ (i.e., $x_{k+1}=A x_k + B u_k, \quad k = 0, \ldots, T-1$) and the cost terms satisfy $Q_k=Q,\forall k$, $R_k=R,\forall k$, the matrices $A, B, Q, R$ in the Riccati equations are all constant for all time steps $k$.
...
\end{align}
The minimum among all these $J_t, t=0,1,2,\cdots,N-1$ provides the optimal solution to the problem and the corresponding $t$ is the optimal horizon.

This result is already shown in optimal-horizon control \cite{stachowicz2021ohmp}.
